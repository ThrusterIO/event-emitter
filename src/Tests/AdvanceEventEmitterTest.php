...
use Thruster\Component\EventEmitter\AdvanceEventEmitter;
use Thruster\Component\EventEmitter\Event;

/**
 * Class AdvanceEventEmitterTest
 *
 * @package Thruster\Component\EventEmitter\Tests
 * @author  Aurimas Niekis <aurimas@niekis.lt>
 */
class AdvanceEventEmitterTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @var AdvanceEventEmitter
     */
    protected $emitter;

    public function setUp()
    {
        $this->emitter = new AdvanceEventEmitter();
    }
    ...
    public function testOnceWithArguments()
    {
        ...
        $this->emitter->once('foo', function ($event) use (&$capturedArgs) {
            $capturedArgs = $event->getParams();
        });

        $this->emitter->emit('foo', new Event(['a', 'b']));
    ...
    public function testEmitWithOneArgument()
    {
        ...
        $this->emitter->on('foo', function ($event) use (&$listenerCalled, $test) {
            $listenerCalled = true;
            
            $test->assertEquals(['bar'], $event->getParams());
        });

        $this->assertSame(false, $listenerCalled);
        $this->emitter->emit('foo', new Event(['bar']));
        $this->assertSame(true, $listenerCalled);
    }
    ...
    public function testEmitWithTwoArguments()
    {
        ...
        $this->emitter->on('foo', function ($event) use (&$listenerCalled, $test) {
            ...
            $test->assertEquals(['bar', 'baz'], $event->getParams());
        });

        $this->assertSame(false, $listenerCalled);
        $this->emitter->emit('foo', new Event(['bar', 'baz']));
        $this->assertSame(true, $listenerCalled);
    }
